...
async function up() {
  ...
  const addCol = async (sql) => { try { await sequelize.query(sql); } catch (e) { /* ignore duplicate */ } };
  await addCol('ALTER TABLE batches ADD COLUMN course_package_id INT;');
  await addCol('ALTER TABLE batches ADD COLUMN faculty_id INT;');
  await addCol('ALTER TABLE batches ADD COLUMN timing VARCHAR(50);');
  await addCol('ALTER TABLE batches ADD COLUMN max_students INT DEFAULT 30;');
  await addCol("ALTER TABLE batches ADD COLUMN status ENUM('Upcoming', 'Active', 'Completed') DEFAULT 'Upcoming';");
  await addCol('ALTER TABLE batches ADD CONSTRAINT fk_batch_cp FOREIGN KEY (course_package_id) REFERENCES course_packages(id);');
  await addCol('ALTER TABLE batches ADD CONSTRAINT fk_batch_fac FOREIGN KEY (faculty_id) REFERENCES users(id);');
